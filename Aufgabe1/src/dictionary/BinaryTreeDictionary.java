...
public class BinaryTreeDictionary<K extends Comparable<? super K>, V> implements Dictionary<K, V> {
    ...
    private Node<K,V> parentOfLeftMostAncestor(Node<K,V> p) { //solange er von rechts kommt wird der parent genommen
        assert p != null;
        ...
        return new Iterator<Dictionary.Entry<K, V>>() {
            ...
            public boolean hasNext() {
                if (current == null) {
                    ...
                    if (current.right != null || parentOfLeftMostAncestor(current) != null)
                        return true;
                    return false;
                }
            }
            ...
            public Dictionary.Entry<K, V> next() {
                ...
                if (current == null) {
                    ...
                    if (current.right != null)
                        current = leftMostDescendant(current.right); //kleinster nachfolger im rechten Teilbaum
                    else
                        current = parentOfLeftMostAncestor(current); //gehe aus rechtem Teilbaum zurueck und springe in naechsten rechtsgelegenen Teilbau
                }
                return new Dictionary.Entry<>(current.key, current.value);
            }
        };
    }

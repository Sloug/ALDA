...
public class AdjacencyListDirectedGraph<V> implements DirectedGraph<V> {
    ...
    public boolean containsEdge(V v, V w) {
		return (!succ.isEmpty() && succ.containsKey(v) && succ.get(v).containsKey(w));
    }
	...
	DirectedGraph<V> invert() {
		DirectedGraph returnValue = new AdjacencyListDirectedGraph<>();
		for (Map.Entry<V,Map<V,Double>> vertex : succ.entrySet()) {
			for (Map.Entry<V,Double> edge : vertex.getValue().entrySet()) {
				returnValue.addEdge(edge.getKey(), vertex.getKey(), edge.getValue());
			}
		}
		return returnValue;
	}
	...
	public String toString() {
		...
		for (Map.Entry<V,Map<V,Double>> vertex : succ.entrySet()) {
			for (Map.Entry<V,Double> edge : vertex.getValue().entrySet()) {
				returnValue.append(vertex.getKey() + " --> " + edge.getKey() + " weight = " + edge.getValue() + "\n");
			}
		}
